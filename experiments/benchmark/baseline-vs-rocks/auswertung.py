...
throughputs = {"baseline": Datastore(), "rocks": Datastore()}
...
for filename in os.listdir('.'):
    if filename.endswith('.csv'):
        with open(filename, newline='') as csvfile:
            ...
            db_type = "rocks" if "rocks" in filename else "baseline"
            for row in reader:
...
def create_plot():
    ...
    for i, db_type in enumerate(db_types):
        ...
        bar_tag = "RocksDB" if db_type == "rocks" else "Baseline"
    ...
    ax.set_title('Comparison of Fjall and RocksDB by Throughput')
    ax.legend()
    plt.tight_layout()
    #plt.show()
    plt.savefig('throughput_comparison-bs-rocks.png')  # Save the plot as a PNG file
